...
class FullNames extends State
{
    protected function beforeRendering(): void
    {
        $this->menu->text(__(ucwords(greetings() . ', welcome to ' . config('app.name') . '. for enquires? Call ' . getSetting('CUSTOMER_CARE_CALLER_ID'))))
            ->lineBreak(1)
            ->text($this->record->get('fullName') ? __('Input entered is not valid! Please enter your full name to continue') : __('Please enter your full name to continue'));
    }

    protected function afterRendering(string $argument): void
    {
        $this->record->set('fullName', strtolower(trim($argument)));
