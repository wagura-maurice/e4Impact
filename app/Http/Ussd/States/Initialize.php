...
class Initialize extends State
{
    ...
    {
        $this->menu->text(__(ucwords(greetings() . ' ' . Cache::get($this->record->get('sessionId'))->user->first_name . ', welcome to ' . config('app.name') . '. for enquires? Call ' . getSetting('CUSTOMER_CARE_CALLER_ID'))))
            ->lineBreak(1)
